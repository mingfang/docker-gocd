  ...
  class PipelinesController < AdminController
    ...
    def create
      ...
      save_action = Class.new(::ConfigUpdate::SaveAction) do
        ...
        def initialize params, user, security_service, pipeline, package_definition_service
      ...
      end.new(params, current_user, security_service, pipeline, package_definition_service)
      save_page(params[:config_md5], nil, {:action => :new, :layout => 'application'}, save_action, l.string("PIPELINE_SAVED_SUCCESSFULLY")) do
        ...
        assert_load(:all_pipelines, cruise_config_service.getCurrentConfig().getAllPipelineNames())
        load_pause_info
        load_autocomplete_suggestions
    ...
    def default_stage_config
      job_configs = JobConfigs.new([JobConfig.new(CaseInsensitiveString.new("defaultJob"), Resources.new, ArtifactPlans.new, com.thoughtworks.cruise.config.Tasks.new([AntTask.new].to_java(Task)))].to_java(JobConfig))
      StageConfig.new(CaseInsensitiveString.new("defaultStage"), job_configs)
    end

    def empty_pipeline
      PipelineConfig.new(CaseInsensitiveString.new(""), com.thoughtworks.cruise.config.materials.MaterialConfigs.new, [default_stage_config].to_java(StageConfig))
    end
    ...
    def load_config_for_edit
      assert_load(:cruise_config, CLONER.deepClone(cruise_config_service.getConfigForEditing()))
    end
    ...
    def load_autocomplete_suggestions
      load_group_list()
      assert_load :pipeline_stages_json, pipeline_stages_json(cruise_config_service.getCurrentConfig(), current_user, security_service, params)
    end
