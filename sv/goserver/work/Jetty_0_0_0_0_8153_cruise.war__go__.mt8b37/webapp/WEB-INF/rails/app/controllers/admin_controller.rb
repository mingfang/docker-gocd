class AdminController < ApplicationController
  ...
  def save_page(md5, redirect_url, render_error_options_or_proc, save_action, success_message = "Saved successfully.", &load_data)
    ...
    save(md5, render_error_options_or_proc, save_action, success_message, load_data) do |message|
      url = com.thoughtworks.cruise.util.UrlUtil.urlWithQuery(@onsuccess_redirect_uri, "fm", set_flash_message(message, "success"))
      redirect_to(url)
    end
  end
  ...
  def save(md5, render_error_options_or_proc, save_action, success_message, load_data)
    ...
    update_response = cruise_config_service.updateConfigFromUI(save_action, md5, current_user, @update_result)
  ...
  def load_context
    assert_load :config_context, create_config_context(cruise_config_service.registry)
  end
end