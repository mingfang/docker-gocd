...
class ApplicationController < ActionController::Base
  ...
  services(:agent_service, :artifacts_service, :backup_service, :changeset_service, :cruise_cache, :cruise_config_file_dao, :cruise_config_service, :cruise_license_service, :dependency_material_service, :environment_config_service, :environment_service, :environment_service,
           ...
           :system_service, :default_plugin_manager, :command_repository_service, :value_stream_map_service, :admin_service, :config_repository, :package_repository_service, :package_definition_service, :pipeline_sql_map_dao)

  # We are doing this so that we do not have to fix 250 odd specs.
  service_with_alias_name(:cruise_config_service_for_url, "cruiseConfigService")


  def set_current_user
    @user = com.thoughtworks.cruise.server.util.UserHelper.getUserName()
    @user_id = session[com.thoughtworks.cruise.server.util.UserHelper.getSessionKeyForUserId()]
  end
  ...
  def url_for(options = {})
    ...
    unless url = cruise_cache.get(BaseUrlChangeListener::URLS_CACHE_KEY, cache_key)
      url = server_config_service.siteUrlFor(super(options), force_ssl || false)
      cruise_cache.put(BaseUrlChangeListener::URLS_CACHE_KEY, cache_key, url)
    end
    url
  end
  ...
  def view_cache_key
    @view_cache_key ||= com.thoughtworks.cruise.server.ui.ViewCacheKey.new
  end
  ...
  def populate_config_validity
    @config_valid = cruise_config_service.checkConfigFileValid().isValid()
  end

  def set_site_urls_in_thread
    server = cruise_config_service_for_url.getCurrentConfig().server()
  ...
  def populate_health_messages
    @current_server_health_states = server_health_service.getAllValidLogs(cruise_config_service.getCurrentConfig())
  end
