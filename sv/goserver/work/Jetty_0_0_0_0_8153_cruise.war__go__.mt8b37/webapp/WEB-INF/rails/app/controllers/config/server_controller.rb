...
class Config::ServerController < AdminController
  ...
  def update
    ...
    @server_configuration_form = ServerConfigurationForm.new(params[:server_configuration_form])
    if @server_configuration_form.validate(result) &&
            update_server_config(@server_configuration_form.to_license,
                                 @server_configuration_form.to_ldap_config,
  ...
  private
  def update_server_config(license, ldap, password, artifacts_dir, purgeStart, purgeUpto, jobTimeout, should_allow_auto_login, mail_host, site_url, secure_site_url,commandRepositoryLocation, result)
    server_config_service.updateServerConfig(license, mail_host, ldap, password, artifacts_dir, purgeStart, purgeUpto, jobTimeout, should_allow_auto_login, site_url, secure_site_url, commandRepositoryLocation, result, params[:cruise_config_md5])
    result.isSuccessful()
  end
