class StagesController < ApplicationController
  ...
  def config
    @ran_with_config_revision = cruise_config_service.getConfigAtVersion(@stage.getStage().getConfigVersion())

    render_stage
  end

  def config_change
    @changes = cruise_config_service.configChangesFor(params[:later_md5], params[:earlier_md5], result = HttpLocalizedOperationResult.new)
  ...
  def tests
    ...
    unless fragment_exist?(view_cache_key.forFbhOfStagesUnderPipeline(stage.getIdentifier().pipelineIdentifier()), :subkey => view_cache_key.forFailedBuildHistoryStage(stage, @response_format))
      ...
      unless cruise_config_service.stageExists(pipeline_name, stage_name)
      ...
      if cruise_config_service.stageHasTests(pipeline_name, stage_name)
        @failing_tests = shine_dao.failedBuildHistoryForStage(stage_identifier, result)
  ...
  def load_current_config_version
    @current_config_version = cruise_config_service.getCurrentConfig().getMd5()
  end
