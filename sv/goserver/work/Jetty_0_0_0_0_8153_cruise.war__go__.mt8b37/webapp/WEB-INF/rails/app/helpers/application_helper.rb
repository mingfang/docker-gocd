...
module ApplicationHelper
  ...
  def stage_identifier_for_locator(stage_locator_string)
    ...
    com.thoughtworks.cruise.domain.StageIdentifier.new(stage_fragments[0], stage_fragments[1].to_i, stage_fragments[2], stage_fragments[3])
  end
  ...
  def load_from_flash
    GO_MESSAGE_KEYS.each do |key|
      val = flash[key]
      if val
        return com.thoughtworks.cruise.presentation.FlashMessageModel.new(Array(val).join(". ").to_s, key.to_s)
      end
    end
    return nil
  end
  ...
  def mycruise_available?
    cruise_config_service.isSecurityEnabled()
  end

  def use_compressed_js?
    com.thoughtworks.cruise.util.SystemEnvironment.new().use_compressed_js()
  end
  ...
  def is_plugins_enabled?
    system_environment.get(com.thoughtworks.cruise.util.SystemEnvironment.PLUGIN_FRAMEWORK_ENABLED)
  end
