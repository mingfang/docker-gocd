class ServerConfigurationForm
  ...
  attr_accessor :license, :license_user, :hostName, :port, :username, :password, :encrypted_password, :tls, :from, :adminMail, :password_changed,
  ...
  def initialize(attributes)
    @license, @license_user = attributes[:license], attributes[:license_user]
    ...
    @jobTimeout = attributes[:timeoutType] == com.thoughtworks.cruise.config.ServerConfig::NEVER_TIMEOUT ? "0" : attributes[:jobTimeout]
  ...
  def self.from_server_config(server_config)
    license_config, security_config, mail_host = server_config.licenseConfig(), server_config.security(),server_config.mailHost()
    ...
    license_params = {:license => license_config.licenseKey(), :license_user => license_config.user()}
    ...
    ServerConfigurationForm.new(mail_host_params.merge(ldap_config_params).merge(allow_auto_login).merge(license_params).merge(artifacts_params).merge(job_timeout_params).merge(site_url_params).merge(command_repo_location))
  end

  def to_mail_host
    if is_empty_mailhost
      return MailHost.new(com.thoughtworks.cruise.security.CruiseCipher.new)
    end
  ...
  def to_license
    LicenseConfig.new(license, license_user)
  end
  ...
  def ==(other)
    license == other.license &&
            license_user == other.license_user
  ...
  def hash
    license ^ license_user ^ hostName ^ port ^ username ^ password ^ tls ^ from ^ adminMail ^ allow_auto_login ^ ldap_username ^ ldap_uri ^ ldap_password ^ ldap_search_base ^ ldap_search_filter ^ password_file_path
  end
end
